...
const HomeAiProductionLogTest = ({}: IProps) => {
    ...
    const mappingData = (lists, results) => {
        ...
        const newData = results.map((result)=>{
           if(map.get(result.mfrCode)){
               return {...result , pressStatus : map.get(result.mfrCode)}
           }else{
               return result
           }
        })
    ...
    const getApi = async () => {
        try {
            ...
            mappingData(pressList.data.results.content , result.data.info_list)
        }catch (error) {
...
export {HomeAiProductionLogTest};