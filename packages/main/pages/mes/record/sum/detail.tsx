...
const BasicContainer = ({page, keyword, option}: IProps) => {
  ...
  const [detailBasicRow, setDetailBasicRow] = useState<Array<any>>([])
  const [column, setColumn] = useState<Array<IExcelHeaderType>>(columnlist.recordSumList)
  const [detailColumn, setDetailColumn] = useState<Array<IExcelHeaderType>>(columnlist.recordSumDetailList)
  ...
  useEffect(() => {
    if(router.query.sum_data) {
      const sumData = JSON.parse(decodeURI(router.query.sum_data as string))

      setBasicRow([{
        ...sumData,
        avg_confirm_quantities: sumData.avg_confirm_quantity,
        avg_achievement: sumData.avg_achievement_rate
      }])
      setDetailBasicRow([...sumData.records.map(v => {
        let tmpPauseTime = ''

        let random_id = Math.random()*1000;

        if(v.paused_time || v.paused_time === 0){
          let seconds = Number(v.paused_time)
        ...
        return {
          ...v,
          id: random_id,
          ln_id: v.lot_number?.number,
          good_quantity: v.good_quantity,
          poor_quantity: v.poor_quantity,
          poor_quantities: v.confirm_quantity,
          poor_count: v.confirm_quantity,
          achievement: v.achievement_rate,
          start: v.start,
          end: v.end,
          uph: v.uph,
    ...
    <div style={{display: 'flex', }}>
      ...
      <div style={{paddingBottom: 40}}>
        ...
        <ExcelTable
          ...
          setSelectList={setSelectList}
          loadEvent={LoadBasic}
          height={detailBasicRow.length * 40 >= 40*18+56 ? 40*19 : detailBasicRow.length * 40 + 56}
        />
        {/*<PaginationComponent*/}
