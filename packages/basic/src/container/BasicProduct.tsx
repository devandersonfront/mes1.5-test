...
const BasicProduct = ({}: IProps) => {
  ...
  const SaveBasic = async () => {
                ...
                ?.map((tool) => {
                  return {
                    ...
                    tool: {
                      ...
                      code: tool.tool.code,
                      name: tool.tool.name,
                      customer: tool.tool.customerData,
                      additional: tool.tool.additional,
                    },
    ...
    if (
      ...
      codeCheck &&
      processCheck &&
      (bom || basicRow[selectRow].bom_root_id)
    ) {
      let res = await RequestMethod("post", `productSave`, result).catch(
        (error) => {
          return (
            error.data &&
  ...
  const DeleteBasic = async () => {
    ...
    if (haveIdRows.length > 0) {
      deletable = await RequestMethod(
        "delete",
        "productDelete",
        haveIdRows.map((row) => ({ ...row, type: row.type_id }))
      );
